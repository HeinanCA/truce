...
type HPAInfo struct {
	...
	// ManagedByKEDA is true when this HPA was generated by a KEDA ScaledObject.
	// KEDA scales on external triggers (queue depth, lag, cron, ...) that do not
	// depend on requests, so such an HPA is DECOUPLED — but truce labels it as
	// KEDA so the user knows that is expected, not a blind spot.
	ManagedByKEDA bool
	// KEDATriggers lists the ScaledObject's trigger types (e.g. "kafka",
	// "prometheus", "cron"). A "cpu"/"memory" trigger also appears as a Resource
	// metric above and is predicted normally.
	KEDATriggers []string
}
